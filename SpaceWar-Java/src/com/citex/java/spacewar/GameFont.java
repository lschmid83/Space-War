...
public class GameFont {
    ...
    public GameFont(String path) {
        ...
        mSpriteSheet[0] = new SpriteSheet(path + "0.bmp", 16, 16, 40);
        mSpriteSheet[1] = new SpriteSheet(path + "1.bmp", 16, 16, 40);
        mCharacterWidth = new int[2][40];
        try {
            for (int i = 0; i < 2; i++) {        	
                BufferedReader br = new BufferedReader(new FileReader(path + i + ".ini"));
                String value;
                try {
                    for (int a = 0; a < 40; a++) {
                        value = br.readLine();
     ...
     * @param charset - The character set e.g. white or red letters
