...
public class Enemy extends Sprite {
     ...
     */   
    public Enemy(int type, float x, float y, int health, int weapon, int powerup)
    {
    	mX = x;
    	mY = y;
    	mStartX = mX;
     ...
     * @param cam The camera coordinates
     */
    public void draw(GL10 gl, Camera cam, SpriteSheet mEnemySprite, SpriteSheet mBullet) {
    	
		if(!mDestroyed)
		{
			if (mHit)
				mEnemySprite.getFrame(1, 'l').draw(gl, mX - cam.x, mY - cam.y);
			else
				mEnemySprite.getFrame(0, 'l').draw(gl, mX - cam.x, mY - cam.y);	
		}
		
    	for(int i = 0; i < mBulletList.size(); i++)
    	{
   			mBullet.getFrame(mBulletList.get(i).getType() - 1, 'l').draw(gl,  mBulletList.get(i).mX - cam.x, mBulletList.get(i).mY + 1 - cam.y);    
    	}	
    }
	...
	{
    	if(mType == 0)
    		return new Rectangle(mX + 2, mY + 3, 12, 12);
    	else if(mType == 1)
    		return new Rectangle(mX + 5, mY + 6, 21, 21);
    	else if(mType == 2)
    		return new Rectangle(mX + 7, mY + 4, 18, 26);
    	else if(mType == 3)
    		return new Rectangle(mX + 1, mY + 2, 30, 31);
    	else if(mType == 4)
    		return new Rectangle(mX + 8, mY + 11, 48, 42);
    	else if(mType == 5)
    		return new Rectangle(mX + 5, mY + 8, 55, 53);
    	else if(mType == 6)
    		return new Rectangle(mX, mY + 4, 32, 26);
    	else if(mType == 7)
    		return new Rectangle(mX + 3, mY + 3, 26, 28);
    	else if(mType == 8)
    		return new Rectangle(mX, mY, 32, 32);
    	else if(mType == 9)
    		return new Rectangle(mX + 3, mY + 3, 26, 28);
    	else if(mType == 10)
    		return new Rectangle(mX + 13, mY + 5, 47, 57);
    	else
