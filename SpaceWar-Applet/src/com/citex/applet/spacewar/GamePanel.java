...
public class GamePanel extends JPanel implements KeyListener, MouseListener,
	...
	private static int mLevelLoadingTimer;  
	...
	public void preloadGraphics()
	{
		...
		mEnemyBulletSprite = new SpriteSheet("res/spr/enemy_bullet.bmp", 32, 32, 2);	
    ...
    public void paintComponent(Graphics g) {
    	...
    	if (Settings.State.equals("license")) //draw license screen
		{
			...
			back_buffer_graphics.drawImage(mCompanyLogo,  53, 78, null);
			mLevelLoadingTimer++;
			if (mLevelLoadingTimer == 5) {
				preloadGraphics();
				mUserInterface = new UserInterface();
			}			
			else if (mLevelLoadingTimer == 60) 
			{
				showTitleScreen();					
			}
		}
		...
		else if(Settings.State.equals("exit")) //exit game show title screen
		{
			setHighScore(mPlayer.getScore());
			showTitleScreen();		
		}
		else if(Settings.State.equals("game over")) //exit game show title screen
		{
			mUserInterface.drawGameOver(back_buffer_graphics, 0);
		}
		else if(Settings.State.equals("game over high score")) //exit game show title screen
		{
